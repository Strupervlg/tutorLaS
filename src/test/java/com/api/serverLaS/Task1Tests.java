...
class Task1Tests {
	...
	static Stream<Arguments> provideTestAnswerTaskCases() {
		return Stream.of(
				Arguments.of("11.ttl", "step3", "step4", "VariableA", true),
				Arguments.of("11.ttl", "step3", "step2", "VariableA", false),
				Arguments.of("11.ttl", "step3", "step21", "VariableA", false),
				Arguments.of("12.ttl", "step4", "step2", "VariableA", true)
		);
	}

	static Stream<Arguments> provideTestHintTaskCases() {
		return Stream.of(
				Arguments.of("11.ttl", "step3", "step4", "VariableA", "В 4 строке трассы переменная \"a\" существует, так как находится в той же области функции main, что и строка создания переменной."),
				Arguments.of("12.ttl", "step4", "step2", "VariableA", "В 2 строке трассы переменная \"a\" существует, так как является глобальной или статической и существует в течение всего времени выполнения программы. ")
		);
	}

	static Stream<Arguments> provideTestErrorTaskCases() {
		return Stream.of(
				Arguments.of("11.ttl", "step3", "step2", "VariableA", "Выбранная строка трассы находится до создания переменной \"a\".<br>"),
				Arguments.of("11.ttl", "step3", "step21", "VariableA", "В выбранной строке трассы переменная \"a\" не существует, так как находится за пределом области функции main, в которой создается переменная.<br>")
		);
	}
	...
	static Stream<Arguments> provideCheckForAllTaskCases() {
		return Stream.of(
				...
				Arguments.of("11_forAll2.ttl", "step3", "VariableA", "В 15 строке трассы переменная \"a\" существует, так как находится в той же области функции main, что и строка создания переменной.<br>"),
				Arguments.of("11_forAll3.ttl", "step3", "VariableA", "В 5 строке трассы переменная \"a\" существует, так как находится в той же области функции main, что и строка создания переменной.<br>В 15 строке трассы переменная \"a\" существует, так как находится в той же области функции main, что и строка создания переменной.<br>"),
	...
	void testAnswerTask(String nameTtl, String stepVar, String step, String var, boolean expResult) {
